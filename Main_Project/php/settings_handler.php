...
if(isset($_POST['update_details_client']))
{
    ...
    {   $username_db=$first.'_'.$last.'_';
        ...
        if($query)
                echo $message.'done1';
        $query = mysqli_query($con, "UPDATE `administration` SET `user_name_db`='$username_db',`email_db`='$email' WHERE `cust_id_db`='$user_id'");
        $message = "Details updated!"."<br>";
        if($query)
                echo $message.'done2';
    }
    ...
    {   $username_db=$first.'_'.$last.'_';
        //$query = mysqli_query($con, "UPDATE `client` SET `first_name_db`='$first', `last_name_db`='$last',`email_db`='$email' WHERE `cust_id_db`='$user_id'");
        $query = mysqli_query($con, "UPDATE `employee` SET `first_name_db`='$first', `last_name_db`='$last',`charges_db`='$charges' ,`email_db`='$email 'WHERE`emp_id_db`='$user_id' ");
        $message = "Details updated!"."<br>";
        if($query)
                echo $message.'done3';
                else
        ...
        $query = mysqli_query($con, "UPDATE `administration` SET `username_db`='$username_db',`email_db`='$email' WHERE `cust_id_db`='$user_id'");
        $message = "Details updated!"."<br>";
        if($query)
                echo $message.'done4';
                else
